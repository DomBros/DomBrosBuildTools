...
Task StaticCodeAnalysis {
    ...
    if (!(Test-Path -Path $scriptStylePath)) {
        $scriptStylePath = "$PSScriptRoot\DomBrosScriptingStyle.psd1"
    }
...
Task RegenerateWiki {
    ...
    # make sure you're in folder with propert casing (e.g. you've run 'cd c:\work\DomBrosTools' instead of 'cd c:\work\dombrostools',
    # otherwise git links might break
    New-MarkdownDoc -ModulePath $env:BHModulePath `
        -OutputPath "$wikiPath\api" `
        -GitBaseUrl "https://github.com/DomBros/$($env:BHProjectName)/blob/master/$($env:BHProjectName)"
}